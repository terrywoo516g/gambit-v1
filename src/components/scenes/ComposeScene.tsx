...
import { useEffect, useState, useRef } from 'react'
  ...
  import { Copy, Pencil, RotateCcw, Pin, Upload, Paperclip } from 'lucide-react'
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-3xl mx-auto h-full bg-white border border-gray-200 rounded-2xl flex flex-col overflow-hidden">
            <textarea 
              value={finalDraft} 
              onChange={e => setFinalDraft(e.target.value)} 
              className="w-full flex-1 p-8 text-[13px] leading-relaxed text-ink resize-none outline-none font-mono"
            />
          </div>
        </div>
      </div>
        ...
        <div className="w-full flex flex-col bg-gray-50/50 p-4 gap-4 overflow-y-auto">
          {modelOutputs.map((output, idx) => (
            <div key={idx} className="bg-white border border-gray-200 rounded-xl flex flex-col shadow-sm shrink-0">
              ...
              <div className="p-4 overflow-y-auto max-h-[300px] text-xs text-ink font-mono whitespace-pre-wrap leading-relaxed">
                {output.content}
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
              ...
              <div className="pt-4 border-t border-gray-100 mt-2">
                <div className="flex items-center justify-between mb-2">
                  <label className="flex items-center gap-2 text-xs font-medium text-ink"><span className="w-5 h-5 rounded bg-gray-400 text-white flex items-center justify-center text-xs"><Paperclip className="w-3 h-3" /></span>自定义文件</label>
