...
interface ReviewSceneProps { workspaceId: string; onDraftGenerated?: (content: string) => void; referencedRunIds?: string[] }

export default function ReviewScene({ workspaceId, onDraftGenerated, referencedRunIds = [] }: ReviewSceneProps) {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        setLoading(true)
        const res = await fetch(`/api/workspaces/${workspaceId}/scenes/review/init`, {
          method: 'POST',
