...
import { FileCheck, Upload, ClipboardPaste, Loader2} from 'lucide-react'
...
export default function ReviewScene({ workspaceId }: ReviewSceneProps) {
  ...
  function handleImportFromDraft() {
    // Dispatch an event to get text from editor
    // We can use a trick: request editor text
      ...
      <div className="flex-1 overflow-y-auto p-4 flex gap-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl flex flex-col shadow-sm flex-1">
            <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50 rounded-t-xl flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleImportFromDraft} className="text-xs text-inkLight hover:text-accent flex items-center gap-1"><ClipboardPaste className="w-3 h-3" />从最终稿导入</button>
              </div>
            </div>
        ...
        <div className="w-1/2 flex flex-col gap-3">
          {hasReviewed ? (
            ...
            <div className="bg-gray-50 border border-gray-200 border-dashed rounded-xl flex flex-col shadow-sm flex-1 items-center justify-center text-inkLight text-sm p-8 text-center">
