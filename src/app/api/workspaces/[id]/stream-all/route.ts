import { NextRequest } from 'next/server'
import { prisma } from '@/lib/db'
import { streamChat } from '@/lib/llm-client'
import { getModelByName } from '@/lib/model-registry'
import { MODEL_REGISTRY as CLIENT_REGISTRY } from '@/lib/llm-client'
import { dbWrite } from '@/lib/db-queue'

export const dynamic = 'force-dynamic'
export const maxDuration = 300 // 防止 Vercel 等平台过早中断
export const runtime = 'nodejs'

const STREAM_TIMEOUT = 60_000 // 60s 无数据超时
const MAX_RETRIES = 1         // 服务端自动重试 1 次

export async function GET(
  req: NextRequest,
  { params }: { params: { id: string } }
) {
  const { id: workspaceId } = params

  // ── 1. 并行读取 workspace 和 runs ──
  const [workspace, allRuns] = await Promise.all([
    prisma.workspace.findUnique({ where: { id: workspaceId } }),
    prisma.modelRun.findMany({ where: { workspaceId } }),
  ])

  if (!workspace || !allRuns || allRuns.length === 0) {
    return new Response(
      `data: ${JSON.stringify({ type: 'error', error: 'Workspace or runs not found' })}\n\n`,
      { status: 404, headers: sseHeaders() }
    )
  }

  const encoder = new TextEncoder()
  let closed = false

  const body = new ReadableStream({
    start(controller) {
      // 核心修复：发送一个 2KB 的空注释，强制冲刷 Nginx/Vercel 等代理服务器的初始缓冲区
      try {
        controller.enqueue(encoder.encode(`: ${' '.repeat(2048)}\n\n`))
      } catch {
        // 忽略写入失败
      }

      ;(async () => {
        // 使用 Promise.all 并行处理该 workspace 下所有的 run
        await Promise.all(
          allRuns.map(async (run) => {
            const runId = run.id

            // ── 已完成 → 直接返回缓存 ──
            if (run.status === 'completed' && run.content) {
              safeSend(controller, {
                type: 'done',
                runId,
                content: run.content,
                tokens: run.tokens,
              })
              return
            }

            // ── 已失败 → 直接返回错误 ──
            if (run.status === 'failed') {
              safeSend(controller, {
                type: 'error',
                runId,
                message: run.error || 'Previous run failed',
              })
              return
            }

            // ── 查模型信息 ──
            const modelInfo = getModelByName(run.model)
            if (!modelInfo) {
              safeSend(controller, {
                type: 'error',
                runId,
                message: `Unknown model: ${run.model}`,
              })
              return
            }

            let fullContent = ''
            let tokensIn = 0
            let tokensOut = 0

            for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
              try {
                if (attempt > 0) {
                  safeSend(controller, { type: 'retry', runId, attempt })
                }

                fullContent = ''
                tokensIn = 0
                tokensOut = 0

                // 带超时的流式调用
                const timeoutCtrl = new AbortController()
                let timer: ReturnType<typeof setTimeout> | null = null

                const resetTimer = () => {
                  if (timer) clearTimeout(timer)
                  timer = setTimeout(() => {
                    timeoutCtrl.abort()
                  }, STREAM_TIMEOUT)
                }
                resetTimer()

                const stream = streamChat({
                  model: modelInfo.apiId,
                  messages: [{ role: 'user', content: workspace.prompt }],
                  provider: (CLIENT_REGISTRY[run.model]?.provider || modelInfo.provider?.toLowerCase() === 'volcano' ? 'volcano' : 'qiniu') as any,
                })

                for await (const chunk of stream) {
                  if (closed) return // 客户端断开，直接退出该 run 的处理

                  if (chunk.type === 'token') {
                    resetTimer()
                    fullContent += chunk.data
                    safeSend(controller, { type: 'token', runId, token: chunk.data })
                  } else if (chunk.type === 'done') {
                    tokensIn = chunk.data.tokensIn ?? 0
                    tokensOut = chunk.data.tokensOut ?? 0
                  } else if (chunk.type === 'error') {
                    throw new Error(String(chunk.data) || 'Stream error from LLM')
                  }
                }

                if (timer) clearTimeout(timer)

                // 成功完成
                safeSend(controller, {
                  type: 'done',
                  runId,
                  content: fullContent,
                  tokens: tokensIn + tokensOut,
                })

                // 异步更新数据库（不阻塞客户端）
                dbWrite(async () => {
                  await prisma.modelRun.update({
                    where: { id: runId },
                    data: {
                      status: 'completed',
                      content: fullContent,
                      tokens: tokensIn + tokensOut,
                      completedAt: new Date(),
                    },
                  })
                }).catch((e) => console.error('[db update error]', e))

                break // 成功，退出重试循环
              } catch (err: any) {
                const isLastAttempt = attempt >= MAX_RETRIES
                console.error(
                  `[stream attempt ${attempt + 1}/${MAX_RETRIES + 1}] ${run.model}:`,
                  err?.message || err
                )

                if (isLastAttempt) {
                  // 最后一次也失败了
                  safeSend(controller, {
                    type: 'error',
                    runId,
                    message: err?.message || 'Stream failed after retries',
                  })

                  dbWrite(async () => {
                    await prisma.modelRun.update({
                      where: { id: runId },
                      data: {
                        status: 'failed',
                        error: err?.message || 'Unknown error',
                      },
                    })
                  }).catch((e) => console.error('[db update error]', e))
                }
              }
            }
          })
        )

        // 所有并发 run 执行完毕后，检查是否整个 workspace 已完成
        dbWrite(async () => {
          const currentRuns = await prisma.modelRun.findMany({
            where: { workspaceId },
          })
          const allDone = currentRuns.every(
            (r) => r.status === 'completed' || r.status === 'failed'
          )
          if (allDone) {
            await prisma.workspace.update({
              where: { id: workspaceId },
              data: { status: 'completed' },
            })
          }
        }).catch((e) => console.error('[db update error]', e))

        // 全部完成后，发送特殊的 all-done 事件
        safeSend(controller, { type: 'all-done' })

        // 关闭流
        if (!closed) {
          closed = true
          try {
            controller.close()
          } catch {}
        }
      })()
    },

    cancel() {
      // 客户端主动断开时触发
      closed = true
    },
  })

  return new Response(body, { headers: sseHeaders() })

  // ── 工具函数 ──
  function safeSend(
    controller: ReadableStreamDefaultController,
    data: Record<string, any>
  ) {
    if (closed) return
    try {
      controller.enqueue(encoder.encode(`data: ${JSON.stringify(data)}\n\n`))
      controller.enqueue(encoder.encode('\n'))
    } catch (e) {
      console.error('[stream send error]', e)
      closed = true
    }
  }
}

function sseHeaders() {
  return {
    'Content-Type': 'text/event-stream',
    'Cache-Control': 'no-cache, no-transform',
    Connection: 'keep-alive',
    'X-Accel-Buffering': 'no',
    'Content-Encoding': 'none',
  }
}
