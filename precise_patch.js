...
// Remove summary cache logic cleanly
...
// Grid replacement (easier to target correctly without breaking map)
const oldGridBlockStart = '<div key={run.id} id={\'run-\' + run.id}\n                          className={`bg-white border rounded-2xl flex flex-col shadow-sm transition ${'
const oldGridBlockEnd = '</div>\n                            </div>\n                          )}\n                        </div>'

const oldExpandedBlockStart = '<div className="bg-white border border-gray-200 rounded-2xl flex flex-col shadow-sm" style={{ minHeight: \'180px\', maxHeight: \'320px\' }}>'
const oldExpandedBlockEnd = '</div>\n                              </div>\n                            )}\n                          </div>'

// We can just use standard indexOf and slicing
const replaceBlock = (str, startStr, endStr, replacement) => {
...
code = replaceBlock(code, oldExpandedBlockStart, oldExpandedBlockEnd, '<AICard run={run} status={status} content={content} activeRunId={activeRunId} referencedRunIds={referencedRunIds} retryRun={retryRun} toggleRef={toggleRef} />')

code = replaceBlock(code, oldGridBlockStart, oldGridBlockEnd, '<AICard key={run.id} run={run} status={status} content={content} activeRunId={activeRunId} referencedRunIds={referencedRunIds} retryRun={retryRun} toggleRef={toggleRef} />')
