...
curl -s -L -c /tmp/ADMIN_JAR -b /tmp/ADMIN_JAR -X POST https://gambits.top/api/auth/callback/credentials \
...
curl -s -L -c /tmp/USER_JAR -b /tmp/USER_JAR -X POST https://gambits.top/api/auth/callback/credentials \
  -H 'Content-Type: application/x-www-form-urlencoded' \
