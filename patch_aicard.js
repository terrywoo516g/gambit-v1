...
const insertPos = content.indexOf('type StepKey')
content = content.slice(0, insertPos) + aiCardCode + content.slice(insertPos)

// Replace mapping block 1
content = content.replace(/<div className="bg-white border border-gray-200 rounded-2xl flex flex-col shadow-sm" style={{ minHeight: '180px', maxHeight: '320px' }}>[\s\S]*?加入最终稿\s*<\/button>\s*<\/div>\s*<\/div>\s*\)}/g, 
...
// Replace mapping block 2
content = content.replace(/<div key={run.id} id={'run-' \+ run.id}[\s\S]*?className={\`bg-white border rounded-2xl flex flex-col shadow-sm transition \${[\s\S]*?加入最终稿\s*<\/button>\s*<\/div>\s*<\/div>\s*\)}/g, 
